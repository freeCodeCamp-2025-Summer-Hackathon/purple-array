	...
	return (
		<div className="min-h-screen pt-8">
			<header className="flex justify-between" >
				...
				{response ?
                    <Link to={'/edit'} className="btn btn-sm md:btn-md">Edit</Link>
                    :
                    <Link to={'/create'} className="btn btn-sm md:btn-md">Create</Link>    
            }
			</header>
			<br />
                ...
                <div
                    ...
                    <div className="flex flex-col lg:flex-row gap-6 max-w-4xl drop-shadow-xl">
                        ...
                        <div
                            ...
                            style={{backgroundImage: 'url("../../../assets/parchmentPaper.png")'}}>
                            <div className="min-h-28 pb-5 m-3 space-y-2">
                                <h3 className="w-full text-lg font-semibold text-secondary">How did you use today's word?</h3>
                                {response?.response ?
                            ...
                            <div className="min-h-28 m-3 space-y-2">
                                {response?.optionalPrompt1 ?
                                    <h3 className="text-lg font-semibold text-secondary">{ response.optionalPrompt1 }</h3>
                                    :
                                    <h3 className="text-lg font-semibold text-secondary">Additional Prompt 1</h3>
                                }
                                {response?.response1 ?
                        ...
                        <div
                            ...
                            <div className="min-h-28 pb-5 m-3 space-y-2">
                                {response?.optionalPrompt2 ?
                                    <h3 className="text-lg font-semibold text-secondary">{ response.optionalPrompt2 }</h3>
                                    :
                                    <h3 className="text-lg font-semibold text-secondary">Additional Prompt 2</h3>
                                }
                                {response?.response2 ?
                            ...
                            <div className="min-h-28 pb-5 m-3 space-y-2">
                                {response?.optionalPrompt3 ?
                                    <h3 className="text-lg font-semibold text-secondary">{ response.optionalPrompt3 }</h3>
                                    :
                                    <h3 className="text-lg font-semibold text-secondary">Additional Prompt 3</h3>
                                }
                                {response?.response3 ?
            ...
            {response &&
                ...
                <Link to={'/'} className="btn btn-sm md:btn-md">Delete</Link>
            </footer>
            }
		</div>
	);
